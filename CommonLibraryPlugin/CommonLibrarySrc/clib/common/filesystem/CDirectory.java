...
public class CDirectory extends CFileElement {
	...
	protected boolean delete(boolean message) {
		deleteInside(message);
		return super.delete(message);
	}

	public boolean deleteInside() {
		return deleteInside(false);
	}

	public boolean deleteInside(boolean message) {
	...
	public CFileElement copyTo(CDirectory toDir, CPath path) {
		CDirectory newDir = toDir.findOrCreateDirectory(path);
		for (CDirectory dir : getDirectoryChildren()) {
			dir.copyTo(newDir);
		}
		for (CFile file : getFileChildren()) {
			file.copyTo(newDir);
		}
		return newDir;
	}
