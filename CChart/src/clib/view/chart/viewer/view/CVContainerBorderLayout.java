...
/**
 * Class CVContainerBorderLayout.
 * 
 * @author macchan
 * @version $Id: CVContainerBorderLayout.java,v 1.1 2005/03/08 03:04:44 bam Exp
 *          $
 */
public class CVContainerBorderLayout implements LayoutManager2,
	 ...
	 */
	public CVContainerBorderLayout() {
	 ...
	 *            the vertical gap.
	 */
	public CVContainerBorderLayout(int hgap, int vgap) {
	...
	public void layoutContainer(Container target) {
		...
		synchronized (target.getTreeLock()) {
			...
			if ((c = getChild(NORTH, ltr)) != null) {
				...
				Dimension d = c.getPreferredSize();
				c.setBounds(left, top, right - left, (int) d.getHeight());
				top += d.getHeight() + vgap;
			}
			if ((c = getChild(SOUTH, ltr)) != null) {
				// Matsu
				int oldleft = left;
				int west_width = west.getPreferredSize().width;
				left = left + west_width;
				left = left + 1; // ScrollPaneのBorderの分を足す
				// !Matsu
				c.setSize(right - left, c.getHeight());
				Dimension d = c.getPreferredSize();
				c.setBounds(left, bottom - (int) d.getHeight(), right - left,
						(int) d.getHeight());
			...
			if ((c = getChild(EAST, ltr)) != null) {
				...
				Dimension d = c.getPreferredSize();
				c.setBounds(right - (int) d.getWidth(), top,
						(int) d.getWidth(), bottom - top);
			...
			if ((c = getChild(WEST, ltr)) != null) {
				...
				// int scrollbarh =
				// ((JScrollPane)center).getHorizontalScrollBar().getHeight();
				// bottom = bottom - scrollbarh;
				// }
				bottom = bottom - 1; // ScrollPaneのBorderの分を引く
				// !Matsu
				c.setSize(c.getWidth(), bottom - top);
				Dimension d = c.getPreferredSize();
				c.setBounds(left, top, (int) d.getWidth(), bottom - top);
